...
### ⑦LEDS、KEYS
#### 7.1开发板上的LEDs和KEYs
开发板上有两组LED和KEY。第一组是PL端的LED和KEY，第二组是PS端的LED和KEY：\
![](../.images_for_documents/48.png)
#### 7.2在Linux中使用LEDs和KEYs
使用LEDs和KEYs实际上就是配置和使用GPIO，在Linux中只需要知道GPIO号就可以使用对应的IO口。PS_LED的GPIO号为376，PS_KEY的GPIO号为365，PL_LED的GPIO号为506，PL_KEY的GPIO号为507。\
